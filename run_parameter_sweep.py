    ...
    for thickness in thicknesses:
        for di, start_distance in enumerate(distances):
            for energy_level in energy_levels:
                ...
                # --------------set up data naming---------------
                fname_tag = f"TEST{mask_config[1]}-{round(thickness)}-{di}"
                fname = f"../simulation-data/secondaries/{fname_tag}_{n_particles:.2E}_{energy_type}_{round(energy_level)}.csv"
                ...
                myhits.get_det_hits(remove_secondaries=True)

                total_primary_hits = len(myhits.hits_dict["Position"])

                myhits_total = Hits(fname=fname, experiment=False)
                myhits_total.get_det_hits(remove_secondaries=False)
                total_hits = len(myhits_total.hits_dict["Position"])

                array = np.array([total_hits, total_primary_hits])

                results_dir = "../simulation-results/secondaries/"
                results_tag = (
                    f"{fname_tag}_{n_particles:.2E}_{energy_type}_{round(energy_level)}"
                )
                results_save = results_dir + results_tag

                np.savetxt(results_save + "-hits.txt", array, delimiter=" ", fmt="%s")
                """
