...
def read_csv_in_parts(
    ...
    sections = num_lines // 25
    raw_hits = np.zeros(
        (n_elements_original * multiplier, n_elements_original * multiplier)
    )
    for ti, ii in enumerate(range(sections, num_lines, sections)):
        ...
        # myhits = Hits(fname=results_save + "_raw.txt", experiment=False, txt_file=True)
