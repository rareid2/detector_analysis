...
def my_conversion(data):
    # i think you should go through each coordinate, find the respsective r and phi
    # THEN just add them all up right?
    x, y = index_coords(data)
    r, phi = cart2polar(x, y)

    # convert r to theta

    # use the FWHM to inform the bin size for theta (not uniform)
    # bin size in phi is uniform
    print(r, theta)
