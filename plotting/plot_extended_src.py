...
for i, n in enumerate(n_p):
    for j, theta in enumerate(thetas):
        ...
        axs[j,i].text(1.3, 1.05, rf'$\times 10^{int(power)}$', ha='right', va='top', transform=axs[j,i].transAxes, fontsize=8)
        

# Add subtitles to the left of each row
row_titles = [r"$\sim$8$\times$10$^6$cm$^{-2}$sr$^{-1}$", r"$\sim$2$\times$10$^6$cm$^{-2}$sr$^{-1}$", r"$\sim$8$\times$10$^5$cm$^{-2}$sr$^{-1}$"]
for j in range(3):
    fig.text(
        0.23 + j * 0.265,
        0.885,
...
plt.savefig("../simulation-results/final-images/6_circles.png",dpi=500,bbox_inches='tight',pad_inches=0.02)
