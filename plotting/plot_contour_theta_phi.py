...
# import FWHM
params = [4.04753095e-04, 2.89058707e+00]

data = np.loadtxt("../simulation-results/rings/89-4.49-ring-pt_1.00E+08_Mono_100_dc.txt")
...
while pixel_size*fwhm_step < max_rad_dist:
    ...
    radial_distances.append(radial_distance_1 + (radial_distance_2 - radial_distance_1)/2)
bins.insert(0,0)
...
gryophase_bins = {}
factors = [1, 2, 3, 4, 5, 6, 8, 9, 10, 12, 15, 18, 20, 24, 30, 36, 40, 45, 72, 90, 120, 180, 360]
for bi,radial_distance in enumerate(radial_distances):
    ...
    if n_pix > 0:
        ...
        gryophase_bins[f"{bi}-indices"] = {f"{key}": [] for key in range(len(gyro_bins)-1)}
...
def custom_formatter(value, _):
    ...
    new_label = round(np.rad2deg(np.arctan(rr / distance)),1) # Replace this with your formula
...
fig, ax = plt.subplots(1,2, figsize=(5.7, 1.75), gridspec_kw={"hspace": 0, "wspace": 0})
...
new_position = [-.09,.11,0.77,0.77]  # [left, bottom, width, height]
...
for pn in range(len(pitch_angle_bins)-1):
    for gi, gb_bin in enumerate(gryophase_bins[f"{pn}-bins"][:-1]):
        ...
        if integrated_val == 0:
            rectangle = plt.Rectangle((x_loc, y_loc), x_width, y_height,  facecolor="white",edgecolor="white")
        else:
            rectangle = plt.Rectangle((x_loc, y_loc), x_width, y_height,  facecolor=color,edgecolor="black",lw=0.2)
...
new_position = [0.49,.065,0.37,0.81]  # [left, bottom, width, height]
ax[1].set_position(new_position)

original_position = cbar.ax.get_position()
print(original_position)

new_position = [0.865,.065,0.92,0.81]  # [left, bottom, width, height]
cbar.ax.set_position(new_position)

circle = plt.Circle((133,133), 6, fill=None, edgecolor='white',linewidth=0.3)
ax[0].add_patch(circle)
circle = plt.Circle((133,133), 1, fill=True, edgecolor='white',linewidth=0.3, facecolor='white')
ax[0].add_patch(circle)
...
plt.savefig("../simulation-results/rings/5_rings.png",dpi=500,bbox_inches='tight',pad_inches = 0.02)
"""
