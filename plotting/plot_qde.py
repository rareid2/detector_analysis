...
plt.plot(qde_energies, qde_curve, color='#0B132B',label='QDE curve for minipix-edu')
plt.plot(energies, np.ravel(qdes), marker='o', alpha=.8, ms=4, linestyle='', color="#564D4F",markeredgewidth=.1)
...
plt.grid()
#plt.legend()
plt.savefig('../experiment_results/QDE_estimates.png',dpi=500)
...
plt.savefig('../experiment_results/activity_vs_distance_cd109.png',dpi=500)
plt.clf()
...
plt.xlabel('distance from source to detector [cm]')
plt.ylabel('counts')
plt.savefig('../experiment_results/counts_vs_distance_cd109.png',dpi=500)
plt.clf()

counts, unc = estimate_counts(emission_peaks_cd109, branching_ratios_cd109, 100, qde_cd109/100, avg_activity*37000)
